...
void limitName(char *s, char *sub, int num) {
    int c = 0;
    while (c < num) {
        sub[c] = s[1+c-1];
        c++;
...
setfakefullscreen(Client *c)
{
    ...
    if (c->isfakefullscreen) {
        ...
        limitName(c->name, limited, 24);
        if (strlen(limited) > 23) {
            dots = "...";
        }
        snprintf(buffer, sizeof(buffer), "notify-send -t 1000 -r 100 \"dwm - fake fulscreen\" \"OFF for %s%s\"", limited, dots);
        system(buffer);
    } else {
        c->isfakefullscreen=1;;
        limitName(c->name, limited, 25);
        if (strlen(limited) > 24) {
            dots = "...";
        }
        snprintf(buffer, sizeof(buffer), "notify-send -t 1000 -r 100 \"dwm - fake fulscreen\" \"ON for %s%s\"", limited, dots);
        system(buffer);
